    ...
    with mid:
        ...
        with tab_login:
            if not st.session_state.show_forgot:
                ...
                fc1, fc2, fc3 = st.columns([1, 1.4, 1])
                with fc2:
                ...
                st.markdown('<p class="small-muted" style="margin-bottom:14px;">'
                            'Verify your shop name to reset your password.</p>',
                            unsafe_allow_html=True)
                with st.form("forgot_form"):
                    fu = st.text_input("Username")
                    fshop = st.text_input("Shop name (as entered at signup)")
                    fp1 = st.text_input("New password", type="password")
                    fp2 = st.text_input("Confirm new password", type="password")
                    freset = st.form_submit_button("Update password", type="primary", use_container_width=True)
                if freset:
                    if not fu or not fshop or not fp1:
                        ...
                        record = db.get_user_by_username(fu.strip())
                        if not record or record.get("shop_name", "").strip().lower() != fshop.strip().lower():
                            st.error("Username and shop name don't match our records.")
                        else:
        ...
        with tab_signup:
            with st.form("signup_form"):
                s_shop = st.text_input("Shop name", placeholder="e.g. Patel General Store")
                s_owner = st.text_input("Shop owner name", placeholder="e.g. Ramesh Patel")
            ...
            if submitted2:
                if not s_shop.strip() or not s_owner.strip() or not nu.strip() or not np1:
                    st.warning("Fill in all fields.")
                elif np1 != np2:
                    st.error("Passwords don't match.")
                elif len(np1) < 4:
                    ...
                    ok, msg = db.create_user(nu.strip(), np1, s_shop.strip(), s_owner.strip())
                    if ok:
...
def main():
    ...
    with st.sidebar:
        shop_name = st.session_state.user.get("shop_name") or "SmartMart"
        owner_name = st.session_state.user.get("owner_name") or ""
        st.markdown(
            f'<div style="font-family:Fraunces,serif;font-weight:700;font-size:1.5rem;'
            f'margin-bottom:2px;">🧾 {shop_name}</div>',
            unsafe_allow_html=True,
        )
        caption = f"Signed in as {st.session_state.user['username']}"
        if owner_name:
            caption = f"{owner_name} · @{st.session_state.user['username']}"
        st.caption(caption)
        st.write("")
        page = st.radio(
            "Navigate",
