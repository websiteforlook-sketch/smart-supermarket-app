...
def chart_style(fig, ax):
    ...
    for spine in ["left", "bottom"]:
        ax.spines[spine].set_color("#DDD5C7")
    ...
    with c2:
        with panel("dash_stock_category"):
            ...
            if products.empty:
                ...
                fig, ax = plt.subplots(figsize=(5, 3.2))
                colors = ["#1B1B18", "#A8532E", "#C98A65", "#4B6B4F", "#9C4A3A", "#6B6558"]
                ax.pie(
                    by_cat.values, labels=by_cat.index, autopct="%1.0f%%",
                    colors=colors[: len(by_cat)], textprops={"color": "#1B1B18", "fontsize": 9},
...
def build_excel_report(products: pd.DataFrame, sales: pd.DataFrame) -> bytes:
    wb = Workbook()
    header_fill = PatternFill(start_color="1B1B18", end_color="1B1B18", fill_type="solid")
    header_font = Font(color="F7F3EC", bold=True)
