...
def init_tables():
    ...
    """)
    # Safe no-op on a fresh table; adds the new columns if this is an
    # existing users table created before shop_name/owner_name existed.
    for ddl in (
        "ALTER TABLE users ADD COLUMN shop_name VARCHAR(150) NOT NULL DEFAULT ''",
        "ALTER TABLE users ADD COLUMN owner_name VARCHAR(150) NOT NULL DEFAULT ''",
    ):
        try:
            cur.execute(ddl)
        except Error:
...
def reset_password(username: str, new_password: str) -> tuple[bool, str]:
    """Set a new password for an existing username (used by 'Forgot password')."""
    conn, cur = _cursor()
    try:
        cur.execute("SELECT id FROM users WHERE username=%s", (username,))
        if not cur.fetchone():
